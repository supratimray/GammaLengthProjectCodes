...
for i=1:size(powerVals,1)
    [h2(i,:),c2] = hist(powerVals(i,:),histCenterVals);
    h2(i,:)=h2(i,:)/sum(h2(i,:));
end
hMean=mean(h2,1);
hErr=std(h2,1)./sqrt(size(powerVals,1));
plot(plotHandle,cN,hN,'r');
plot(cN,hN+hErr,'color','r','linestyle','--');
plot(cN,hN-hErr,'color','r','linestyle','--');
xlim(plotHandle,xLim);

Y = exprnd(1,size(powerVals(:),1),size(powerVals(:),2));
[hT,cT] = hist(Y(:),histCenterVals);
hT=hT/sum(hT);
plot(plotHandle,cT,hT,'color','c','linestyle','--');
xlim(plotHandle,xLim);
end